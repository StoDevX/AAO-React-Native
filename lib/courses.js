...
async function getAllCoursesFromServer(): Promise<{[key: string]: (CourseType|Error)[]}> {
  ...
  if (terms instanceof SisAuthenticationError) {
    return terms
  }
...
async function getSisTermListingsPage(): string|Error|null {
  try {
...
async function getSisCoursesPage(term: number): string|Error|null {
...
function rowToCourse(domRow: Object, clbid: number, term: number): CourseType {
  ...
  let {department, number, section} = parseDeptNum(getText(course.deptnum))
