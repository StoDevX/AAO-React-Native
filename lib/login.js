...
export async function isLoggedIn(): Promise<boolean> {
  ...
  result = result && await AsyncStorage.getItem('olafCredentials:valid')
...
export async function performLogin(username: string, password: string): Promise<{result: boolean, message?: string}> {
  if (username && password) {
    ...
    if (!result) {
      await AsyncStorage.setItem('olafCredentials:valid', JSON.stringify(false))
      return {result, message}
    }

    await Promise.all([
      saveLoginCredentials(username, password),
      AsyncStorage.setItem('olafCredentials:valid', JSON.stringify(true)),
    ])
