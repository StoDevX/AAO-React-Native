...
export async function loadAllCourses(isConnected: boolean, force?: boolean): Promise<CourseCollectionType> {
  ...
  const termsOrError = await loadListOfTerms({stnum, isConnected, force})
  ...
  const courseLists = await Promise.all(terms.map(term =>
    loadCoursesForTerm({term, stnum, isConnected, force})))
