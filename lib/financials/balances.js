...
export async function getFinancialData(force?: bool): PromisedDataType {
  const isConnected = await NetInfo.isConnected.fetch()

  const [flex, ole, print] = await getBalances()
  const isExpired = flex.isExpired || ole.isExpired || print.isExpired
  const isCached = flex.isCached || ole.isCached || print.isCached
