...
function job(id: string, title: string): JobSummary {
	return {id, title, postedDate: '2026-09-01', location: undefined}
}
...
describe('visibleSections', () => {
	test('shows every posting under its category when nothing narrows the list', () => {
		let sections = visibleSections(CATEGORIES, buildJobFilters(ALL_JOBS, NOTHING_CHOSEN), '')
	...
	test('keeps postings at any chosen level', () => {
		...
		expect(ids(visibleSections(CATEGORIES, filters, ''))).toEqual(['1', '3', '4', '6'])
	})

	test('keeps postings in a chosen term', () => {
		let filters = buildJobFilters(ALL_JOBS, {level: null, term: ['Fall']})
		expect(ids(visibleSections(CATEGORIES, filters, ''))).toEqual(['2'])
	})
	...
	test('keeps postings with no stated term when "Not stated" is chosen', () => {
		...
		expect(ids(visibleSections(CATEGORIES, filters, ''))).toEqual(['6'])
	})

	test('requires a posting to match both filters', () => {
		let filters = buildJobFilters(ALL_JOBS, {level: ['Entry-level'], term: ['Academic Year']})
		expect(ids(visibleSections(CATEGORIES, filters, ''))).toEqual(['1', '4'])
	})

	test('drops a category the filters empty', () => {
		let filters = buildJobFilters(ALL_JOBS, {level: null, term: ['Summer']})
		expect(visibleSections(CATEGORIES, filters, '').map((section) => section.title)).toEqual([
			'Summer Student Work',
		])
	})
	...
	test('searches the start of each word in the title', () => {
		let filters = buildJobFilters(ALL_JOBS, NOTHING_CHOSEN)
		expect(ids(visibleSections(CATEGORIES, filters, 'stock'))).toEqual(['7'])
		expect(ids(visibleSections(CATEGORIES, filters, 'ock'))).toEqual([])
	})

	test('requires every word of the search, in any order', () => {
		let filters = buildJobFilters(ALL_JOBS, NOTHING_CHOSEN)
		expect(ids(visibleSections(CATEGORIES, filters, 'assistant chem'))).toEqual(['2'])
	})
	...
	test('does not match the term prefix or pay code a student never sees', () => {
		let filters = buildJobFilters(ALL_JOBS, NOTHING_CHOSEN)
		expect(ids(visibleSections(CATEGORIES, filters, 'ws'))).toEqual([])
		expect(ids(visibleSections(CATEGORIES, filters, 'f26'))).toEqual([])
	})

	test('searches within the filtered postings', () => {
		let filters = buildJobFilters(ALL_JOBS, {level: ['Entry-level'], term: null})
		expect(ids(visibleSections(CATEGORIES, filters, 'student'))).toEqual(['1', '4', '6'])
	})
})
