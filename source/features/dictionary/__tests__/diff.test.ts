...
import {addSense, deleteSense, moveSense, setSenseField, startDraft} from '../lib/draft'
...
describe('diffEntry', () => {
	...
	it('keeps a deleted sense beside the neighbour it actually had, even once that neighbour has moved', () => {
		// before: One, Two, Three. Three moves to the front, then Two is
		// deleted. Two used to sit right after One, so it belongs right after
		// wherever One ends up — not wedged between Three and One, which is
		// where a deletion re-indexed against the pre-move list would land it.
